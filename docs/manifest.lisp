(:docstring-markup-format
   :scriba
   :systems (:MNAS-PACKAGE)
   :documents ((:title "mnas-package"
	        :authors ("Nick Matvyeyev")
	        :output-format (:type :multi-html :template :gamma)
                :sources ("mnas-package.scr" "mnas-package-example.scr"
                          "mnas-package-make.scr" "mnas-package-obj.scr"
                          "mnas-package-pkg.scr" "mnas-package-sec.scr"
                          "mnas-package-sys.scr" "mnas-package-view.scr"
                          "mnas-package-graph.scr"
                          "mnas-package-make-graph.scr"
                          "mnas-package-obj-graph.scr"
                          "mnas-package-pkg-graph.scr"
                          "mnas-package-sec-graph.scr"
                          "mnas-package-sys-graph.scr"
                          "mnas-package-view-graph.scr")
                )))
