...
(def-test obj-name ()
  (let ((pkg (find-package :mnas-package/example)))
    ...
    (is-true (equal (multiple-value-list (mpkg/obj:obj-name #'mnas-package/example:f-a-exp))
                    '(mnas-package/example:f-a-exp :FUNCTION)))
    (is-true (equal (multiple-value-list (mpkg/obj:obj-name #'mnas-package/example:m-a-exp))
    ...
    (map nil #'(lambda (el)
         ...
         (closer-mop:generic-function-methods #'mnas-package/example::m-a-exp))
...
(def-test obj-package ()
  (let ((pkg (find-package :mnas-package/example)))
    ...
    (is-true (eq (mpkg/obj:obj-package #'mnas-package/example:f-a-exp) pkg))
    (map nil #'(lambda (el)
